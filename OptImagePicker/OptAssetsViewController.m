...
#import "OptAssetsViewController.h"
...
@interface OptImagePickerController (Private)

@end

@implementation NSIndexSet (Convenience)

- (NSArray *)Opt_indexPathsFromIndexesWithSection:(NSUInteger)section {
...
- (NSArray *)Opt_indexPathsForElementsInRect:(CGRect)rect {
...
@interface OptAssetsViewController () <PHPhotoLibraryChangeObserver, UICollectionViewDelegateFlowLayout>
...
@implementation OptAssetsViewController
...
- (IBAction)done:(id)sender {
    if (self.imagePickerController.didFinishPickingAssets) {
        self.imagePickerController.didFinishPickingAssets(self.imagePickerController, self.imagePickerController.selectedAssets.array);
    }
}

- (IBAction)back:(id)sender {
    [self.imagePickerController.albumsNavigationController popViewControllerAnimated:YES];
}
...
- (void)updateFetchRequest {
    if (self.assetCollection) {
        ...
        switch (self.imagePickerController.mediaType) {
            case OptImagePickerMediaTypeImage:
                options.predicate = [NSPredicate predicateWithFormat:@"mediaType == %ld", PHAssetMediaTypeImage];
                break;
                
            case OptImagePickerMediaTypeVideo:
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(self.collectionView.bounds) / 3.0) {
        ...
        [self computeDifferenceBetweenRect:self.previousPreheatRect andRect:preheatRect addedHandler:^(CGRect addedRect) {
            NSArray *indexPaths = [self.collectionView Opt_indexPathsForElementsInRect:addedRect];
            [addedIndexPaths addObjectsFromArray:indexPaths];
        } removedHandler:^(CGRect removedRect) {
            NSArray *indexPaths = [self.collectionView Opt_indexPathsForElementsInRect:removedRect];
            ...
            if (![collectionChanges hasIncrementalChanges] || [collectionChanges hasMoves]) {
                ...
                [self.collectionView performBatchUpdates:^{
                    ...
                    if ([removedIndexes count]) {
                        [self.collectionView deleteItemsAtIndexPaths:[removedIndexes Opt_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *insertedIndexes = [collectionChanges insertedIndexes];
                    if ([insertedIndexes count]) {
                        [self.collectionView insertItemsAtIndexPaths:[insertedIndexes Opt_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *changedIndexes = [collectionChanges changedIndexes];
                    if ([changedIndexes count]) {
                        [self.collectionView reloadItemsAtIndexPaths:[changedIndexes Opt_indexPathsFromIndexesWithSection:0]];
                    }
                } completion:NULL];
            }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    OptAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"AssetCell" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.imagePickerController.mediaType) {
            case OptImagePickerMediaTypeAny:
            {
                NSString *format;
                if (numberOfPhotos == 1) {
                    if (numberOfVideos == 1) {
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.imagePickerController.delegate respondsToSelector:@selector(Opt_imagePickerController:shouldSelectAsset:)]) {
        PHAsset *asset = self.fetchResult[indexPath.item];
        return [self.imagePickerController.delegate Opt_imagePickerController:self.imagePickerController shouldSelectAsset:asset];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    OptImagePickerController *imagePickerController = self.imagePickerController;
    ...
    if (imagePickerController.allowsMultipleSelection) {
        ...
        if (imagePickerController.didFinishPickingAssets) {
            imagePickerController.didFinishPickingAssets(imagePickerController, @[asset]);
        }
    }
    
    if ([imagePickerController.delegate respondsToSelector:@selector(Opt_imagePickerController:didSelectAsset:)]) {
        [imagePickerController.delegate Opt_imagePickerController:imagePickerController didSelectAsset:asset];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([imagePickerController.delegate respondsToSelector:@selector(Opt_imagePickerController:didDeselectAsset:)]) {
        [imagePickerController.delegate Opt_imagePickerController:imagePickerController didDeselectAsset:asset];
    }
}
